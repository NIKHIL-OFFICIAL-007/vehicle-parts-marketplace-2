...
try {
    ...
    error_log("Failed to fetch pending role requests count: " . $e->getMessage());
    $pending_role_requests = 0;
}
?>
...
<head>
  ...
  <!-- ✅ Correct Tailwind CDN (no extra spaces) -->
...
<body class="bg-gray-50 text-gray-900">
  ...
  <div class="fixed left-0 top-0 w-72 h-full bg-sidebar-bg text-white">
    ...
    <nav class="mt-4 px-2">
      ...
      <!-- Role Requests with Notification Badge -->
      <a href="role_requests.php" class="flex items-center px-6 py-3 text-gray-300 hover:bg-sidebar-hover hover:text-white rounded-lg transition relative <?= basename($_SERVER['PHP_SELF']) === 'role_requests.php' ? 'bg-sidebar-active text-white' : '' ?>">
